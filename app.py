...
# 🚨 Инициализация session_state ДО использования
...
with st.sidebar:
    ...
    with col1:
        # Кнопка Петра I - используем on_click
...
with st.sidebar.expander("🔧 Отладка"):
    st.write(f"Выбранный персонаж: {st.session_state.vybranniy_personazh}")
    st.write(f"История сообщений: {len(st.session_state.istorija_chata)} сообщений")
    st.write(f"Ключи загружены: {'Да' if API_KEY and folder_id else 'Нет'}")
...
with st.expander("🔧 Техническая информация"):
    ...
    if API_KEY and folder_id:
        ...
        st.write(f"**API Key (первые 10 символов):** `{API_KEY[:10]}...`")
