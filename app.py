...
load_dotenv()  # Автоматически загружает .env из текущей папки

# 🔐 Берем секретные ключи ИЗ .env файла
API_KEY = os.getenv("YANDEX_API_KEY")      # 🗝️ Ключ доступа к Яндекс GPT
folder_id = os.getenv("YANDEX_FOLDER_ID")  # 📁 ID твоего облака в Яндексе

# 🚨 Проверяем что ключи загружены
if API_KEY is None or folder_id is None:
    st.error("""
    ❌ ОШИБКА: Ключи API не найдены!
    
    📝 Создайте файл `.env` в папке с проектом со следующим содержимым:
    ...
    ⚠️ Или установите переменные окружения в системе.
    """)
    st.stop()
...
def poluchit_otvet_yandex(personazh, vopros):
    ...
    try:
        ...
        headers = {
            ...
            "Authorization": f"Api-Key {API_KEY}"    # 🗝️ Наш секретный ключ ИЗ .env
        }
    ...
    # 🚨 Если что-то пошло не так
        ...
        return f"❌ Ошибка соединения с Яндекс GPT: {str(e)}"
    except KeyError as e:
        return f"❌ Ошибка в формате ответа от Яндекс GPT: {str(e)}"
...
with st.sidebar:
    ...
    # 🔧 Техническая информация о загрузке ключей
    with st.expander("🔐 Статус ключей"):
        if API_KEY and folder_id:
            st.success("✅ Ключи успешно загружены из .env файла")
            st.write(f"**Folder ID:** `{folder_id[:10]}...`")
            st.write(f"**API Key:** `{API_KEY[:10]}...`")
        else:
            st.error("❌ Ключи не загружены")
...
with st.expander("🔧 Техническая информация (для любознательных)"):
    ...
    **Как это работает:**
    ...
    - 🔐 **.env файл** - безопасное хранение ключей
    - 🌐 **API** - специальный канал связи с AI
    
    **Безопасность:**
    - ✅ Ключи хранятся в отдельном .env файле
    - ✅ .env файл НЕ загружается в GitHub
    - ✅ Ключи защищены от случайной публикации
    - ✅ Легко менять ключи без изменения кода
    """)
    
    # Показываем статус загрузки .env
    st.write("**Статус загрузки .env:**")
    env_path = os.path.join(os.path.dirname(__file__), '.env')
    if os.path.exists(env_path):
        st.success(f"✅ .env файл найден: `{env_path}`")
    else:
        st.warning(f"⚠️ .env файл не найден. Создайте файл `{env_path}`")
